-- 0013_service_create: audit the onboarding service creation (S24).
-- SQLite cannot alter CHECK constraints, so the audit table is rebuilt
-- (same pattern as 0006 and 0011).
CREATE TABLE audit_new (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  service_id TEXT REFERENCES services(id),
  actor TEXT NOT NULL,
  action TEXT NOT NULL CHECK (action IN ('cutover', 'rollback', 'login', 'deploy', 'db_backup', 'db_restore', 'service_create')),
